...
class IncomeStatement(BaseScrapper):
    ...
    def __init__(self, webpage):
        ...
        for ind, i in enumerate(self._th):
            ...
            self._net_interest_income = self.get_net_interest_income(ind)
            self._bank_interest_income = self.get_bank_interest_income(ind)
            self._total_interest_expense = self.get_total_interest_expense(ind)
            
            self._loan_loss_provision = self.get_loan_loss_provision(ind)

            self._net_interest_income_after_loan_provision = self.get_net_interest_income_after_loan_provision(ind)
            self._bank_non_interest_income = self.get_bank_non_interest_income(ind)
            self._bank_non_interest_expense = self.get_bank_non_interest_expense(ind+1)
            
            self._net_income_before_taxes = self.get_net_income_before_taxes(ind)
            self._provision_for_income_taxes = self.get_provision_for_income_taxes(ind)

            self._net_income_after_taxes = self.get_net_income_after_taxes(ind)
